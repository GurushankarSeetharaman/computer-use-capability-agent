...
def render_partial(text: str, values: dict[str, object]) -> str:
    """Substitute the placeholders we have values for, leave the rest alone.

    Used where only *some* names are resolvable -- discovery knows the
    credentials it was handed and nothing else, and must not choke on a
    placeholder that belongs to a later stage.
    """
